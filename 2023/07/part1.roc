...
main =
    ...
    Stdout.line "Part 1 winnings: \(Num.toStr winnings)"

solve : Puzzle -> Nat
solve = \hands ->
    hands
    |> List.map (\hand -> {hand & type: handType hand})
    |> List.sortWith handComparator
    |> List.walkWithIndex 0 \sum, hand, idx -> sum + (hand.bid * (idx+1))

handComparator : Hand, Hand -> [LT, EQ, GT]
handComparator = \a, b ->
    aType = handTypeValue a.type
    bType = handTypeValue b.type

    if aType < bType then LT
    else if aType > bType then GT
    else cardsComparator a.cards b.cards
...
cardsComparator = \a, b ->
    if a == [] || b == [] then EQ
        ...
        if aCard < bCard then LT
        else if aCard > bCard then GT
        else cardsComparator (List.dropFirst a 1) (List.dropFirst b 1)
...
handType = \hand ->
    ...
    cardCounts : List Nat
    cardCounts = List.walk hand.cards (Dict.empty {})  
        (\counts, card -> Dict.update counts card incrementCardCount)
        |> Dict.values
    
    if cardCounts |> contains 5 then FiveOfAKind
    else if cardCounts |> contains 4 then FourOfAKind
    else if cardCounts |> contains 3 && cardCounts |> contains 2 then FullHouse
    else if cardCounts |> contains 3 then ThreeOfAKind
    else if cardCounts |> List.keepIf (\n -> n == 2) |> List.len == 2 then TwoPair
    else if cardCounts |> contains 2 then OnePair
    else HighCard
