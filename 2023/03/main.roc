...
main =
    ...
    gears = symbols
        ...
        |> List.keepIf \sym -> sym.v == asterisk

        # Now add the list of parts they are touching
        |> List.map \ast -> {
            gear: ast,
            parts: engineParts |> Set.toList
    ...
    totalGearRatio = gears
        # Calculate the ratio of each gear
        |> List.map \gear -> List.map gear.parts .n
        |> List.map \parts -> List.walk parts 1 Num.mul
...
digitsToTheLeftInclusive = \lst, pos ->
    ...
    if !(isDigit chr) then []
    else if pos == 0 then [chr]
    else List.concat (digitsToTheLeftInclusive lst (pos-1)) [chr]
...
digitsToTheRightExclusive = \lst, pos ->
    if (pos + 1) >= List.len lst then []
        ...
        if isDigit chr then List.concat [chr] (digitsToTheRightExclusive lst (pos+1))
        else []

findTouchingDigits : Engine, Symbol -> List Symbol
findTouchingDigits = \engine, sym ->
    locations = [{x: sym.x-1, y: sym.y-1},
                 {x: sym.x,   y: sym.y-1},
                 {x: sym.x+1, y: sym.y-1},
...
range = \from, until ->
    if from == until then [from]
    else List.concat [from] (range (from+1) until)
